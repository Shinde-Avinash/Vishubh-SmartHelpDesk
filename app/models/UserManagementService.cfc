component singleton {
    ...
    function getAgentTeam( required numeric userId ){
    ...
    function assignAgentTeam( required numeric userId, required any teamId ){
        // Check if agent exists
        var check = queryExecute("SELECT id FROM agents WHERE user_id = :uid", { uid: { value: arguments.userId, cfsqltype: "cf_sql_integer" } });
        
        if ( len(arguments.teamId) ) {
            if ( check.recordCount ) {
                queryExecute("UPDATE agents SET team_id = :tid WHERE user_id = :uid", {
                    tid: { value: arguments.teamId, cfsqltype: "cf_sql_integer" },
                    uid: { value: arguments.userId, cfsqltype: "cf_sql_integer" }
                });
            } else {
                queryExecute("INSERT INTO agents (user_id, team_id) VALUES (:uid, :tid)", {
                    tid: { value: arguments.teamId, cfsqltype: "cf_sql_integer" },
                    uid: { value: arguments.userId, cfsqltype: "cf_sql_integer" }
                });
            }
        } else {
            // Optional: Remove from agents if team is cleared? Or just set NULL?
            if ( check.recordCount ) {
                queryExecute("UPDATE agents SET team_id = NULL WHERE user_id = :uid", {
                    uid: { value: arguments.userId, cfsqltype: "cf_sql_integer" }
                });
            }
        }
